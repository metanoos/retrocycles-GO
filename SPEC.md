...
> `tailRadius` is the authoritative frozen tail (1.5–4 m in 0.5 m steps); the
> player collision radius is a fixed 2 m and head-to-trail contact is their sum (decision T);
...
- `CheckCollision(playerPos, prevPos, localPlayerId)` — queries
  ...
  movement and each candidate segment to world space, subdivides long accepted
  sweeps into fixed 4 m microsegments, and evaluates continuous **3D segment-to-
  segment distance**. Contact is inclusive at `tailRadius + 2 m fixed player
  radius`. Fires `OnCollisionDetected(ownerId)` on the first overlapping capsule.
...
- The host spawns **`NetworkMatchState`** (`Resources/Player/NetworkMatchState.prefab`,
  ...
  `[Networked] FrozenTailRadius`, fixed `FrozenPlayerHeadRadiusCm`, and
  `FrozenConfigHash` (decision T). Clients rederive and verify the integer
  config on spawn/change before applying it to their local `ITailAuthority`.
...
- **NetworkMatchState Prefab** (FUSION_WEAVER, Track C, decision Q/T) — minimal
  ...
  at match start; it carries the frozen tail, fixed player radius, and config hash. Not scene-
