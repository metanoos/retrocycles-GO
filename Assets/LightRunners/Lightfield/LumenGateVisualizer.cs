...
namespace LightRunners.Lightfield
{
    ...
    public class LumenGateVisualizer : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            if (_prefab == null)
            {
                Debug.LogWarning($"[LumenGateVisualizer] No gate prefab at Resources/{PrefabPath}; using the zero-art runtime gate.");
            }
            if (_hooked) return;
        ...
        private void OnGateSpawned(int gateIdValue, double lat, double lon, double alt, GatePlacement placement)
        {
            ...
            GameObject go = _prefab != null
                ? Instantiate(_prefab, world, Quaternion.identity)
                : new GameObject("LumenGate", typeof(SphereCollider), typeof(LumenGate));
