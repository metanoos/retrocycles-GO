...
namespace LightRunners.Gameplay.Tests
{
    ...
    public class MatchLoopWiringTests
    {
        ...
        [Test]
        public void RespawnClearance_RequiresConfiguredHorizontalDistance()
        {
            var crash = new GeoPoint(37.7749, -122.4194, 10.0);
            var tooClose = OffsetNorth(crash, 4.9);
            var clear = OffsetNorth(crash, 5.1);

            Assert.IsFalse(MatchManager.HasReachedRespawnClearance(crash, tooClose, 5f));
            Assert.IsTrue(MatchManager.HasReachedRespawnClearance(crash, clear, 5f));
        }

        [Test]
        public void RespawnClearance_IgnoresAltitudeOnlyMovement()
        {
            var crash = new GeoPoint(37.7749, -122.4194, 10.0);
            var directlyAbove = new GeoPoint(crash.latitude, crash.longitude, 100.0);

            Assert.IsFalse(MatchManager.HasReachedRespawnClearance(crash, directlyAbove, 5f));
        }

        [Test]
        public void ContinueFromSummary_ResetsAfterglowCameraLifecycle()
        {
            var afterglowRoot = new GameObject("AfterglowLifecycle", typeof(AfterglowViewController));
            var summaryRoot = new GameObject("RunSummaryLifecycle", typeof(RunSummaryUI));
            try
            {
                var controller = AfterglowViewController.EnsureRuntimeInstance();
                var package = new ReplayPackage("lifecycle", System.DateTime.UtcNow, default);
                package.Freeze();
                controller.ShowOverview(package);
                Assert.AreEqual(AfterglowView.Overview, controller.CurrentView);

                var onContinue = typeof(RunSummaryUI).GetMethod("OnContinue",
                    System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                Assert.IsNotNull(onContinue);
                onContinue.Invoke(summaryRoot.GetComponent<RunSummaryUI>(), null);

                Assert.AreEqual(AfterglowView.None, controller.CurrentView);
                Assert.IsNull(controller.CurrentPackage);
                var overview = controller.GetComponentInChildren<OverviewCameraController>(true);
                Assert.IsNotNull(overview);
                Assert.IsFalse(overview.IsShown);
            }
            finally
            {
                Object.DestroyImmediate(summaryRoot);
                Object.DestroyImmediate(afterglowRoot);
            }
        }

        private static GeoPoint OffsetNorth(GeoPoint origin, double meters)
        {
            double degrees = meters / (System.Math.PI * GeoPoint.EarthRadiusMeters / 180.0);
            return new GeoPoint(origin.latitude + degrees, origin.longitude, origin.altitude);
        }
    }
}
