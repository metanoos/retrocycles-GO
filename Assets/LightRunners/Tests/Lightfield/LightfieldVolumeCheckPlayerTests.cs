...
namespace LightRunners.Lightfield.Tests
{
    ...
    /// degree offsets map to known metre distances. The volume uses the default
    /// <c>GameConfig.lightfieldBaseRadiusMeters=50</c> + <c>lightfieldDomeCeilingMeters=6</c>.
    /// </summary>
    [TestFixture]
    public class LightfieldVolumeCheckPlayerTests
    {
        ...
        public void CheckPlayer_WalkInsideThenOutside_FiresOnce()
        {
            ...
            // Player starts inside at origin; walks outside beyond the 50m disc.
            vol.CheckPlayer("p1", At(0, 0, 0));
            Assert.IsEmpty(fires, "should not fire while inside");
            vol.CheckPlayer("p1", At(60, 0, 0));
            Assert.AreEqual(new[] { "p1" }, fires, "exactly one fire on inside→outside");
        }

        [Test]
        public void CheckPlayer_OutsideInOut_FiresExactlyTwice()
        {
            var fires = CaptureViolations(out var vol);
            vol.CheckPlayer("p1", At(60, 0, 0));   // starts outside — see CheckPlayer_StartsOutside_NoFire
            vol.CheckPlayer("p1", At(0, 0, 0));     // re-enter (no fire)
            vol.CheckPlayer("p1", At(60, 0, 0));    // exit again
        ...
        public void CheckPlayer_StartsOutside_NeverEntered_NoFire()
        {
            var fires = CaptureViolations(out var vol);
            for (int i = 0; i < 10; i++)
                vol.CheckPlayer("p1", At(60, 0, 0)); // outside every tick
        ...
        public void CheckPlayer_StandingOutside_DoesNotSpam()
        {
            ...
            for (int i = 0; i < 10; i++)
                vol.CheckPlayer("p1", At(60, 0, 0)); // standing outside
        ...
        public void CheckPlayer_MultiplePlayers_Independent()
        {
            ...
            vol.CheckPlayer("p1", At(60, 0, 0)); // p1 exits
        ...
        public void CheckPlayer_AltitudeCeiling_FiresOnUpwardExit()
        {
            ...
            vol.CheckPlayer("p1", At(0, 0, 7));    // above ceiling (default 6m)
