...
namespace LightRunners.Tests.Backend
{
    ...
    public class MatchSchemaTests
    {
        ...
        public void Match_RPCs_Are_Security_Definer()
        {
            ...
            Assert.GreaterOrEqual(definerCount, 3,
                "create_match/record_match_result/finalize_match must all be SECURITY DEFINER.");
        }
        ...
        [Test]
        public void RecordMatchResult_Enforces_Host_Or_Self()
        {
            // record_match_result must reject non-host writes for other players.
            // The 'not_host' exception token is the client-visible signal.
            Assert.That(_sql, Does.Contain("not_host"),
                "record_match_result / finalize_match must raise 'not_host' for unauthorized writers.");
        }
